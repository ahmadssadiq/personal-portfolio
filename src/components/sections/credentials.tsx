...
const CredentialsSection = ({ showTennis, showBoston, setShowBoston, setShowTennis, setShowPakistan, setShowQatar }: CredentialsSectionProps) => {
  return (
    <div className="mt-6 break-words overflow-wrap-anywhere">
      <p className={`mb-1 text-sm font-normal transition-colors duration-300 ${(showTennis || showBoston) ? 'text-muted opacity-70' : 'text-muted'}`}>
      ...
      <p className={`m-0 text-sm font-normal transition-colors duration-300 ${(showTennis || showBoston) ? 'text-muted opacity-70' : 'text-muted'}`}>
