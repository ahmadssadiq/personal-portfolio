...
const MainContent = ({ showTennis, setShowTennis }: MainContentProps) => {
  return (
    <div className="flex flex-col space-y-4 break-words overflow-wrap-anywhere">
      <p className={`text-sm leading-relaxed transition-colors duration-300 ${showTennis ? 'text-gray-300' : 'text-foreground text-muted'}`}>
      ...
      <p className={`text-sm leading-relaxed transition-colors duration-300 ${showTennis ? 'text-gray-300' : 'text-foreground text-muted'}`}>
        I love <span className="text-sm text-foreground">football </span> (in other words 'soccer'), <button onClick={() => setShowTennis(!showTennis)} className="text-sm text-foreground hover:underline cursor-pointer">tennis</button>, and <span className="text-sm text-foreground">padel</span>.
      </p>
      <p className={`text-sm leading-relaxed transition-colors duration-300 ${showTennis ? 'text-gray-300' : 'text-foreground text-muted'}`}>
