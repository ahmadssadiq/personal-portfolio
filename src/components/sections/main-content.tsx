...
const MainContent = ({ showTennis, setShowTennis, showPakistan, setShowPakistan, showQatar, setShowQatar }: MainContentProps) => {
  return (
    <div className="flex flex-col space-y-4 break-words overflow-wrap-anywhere">
      ...
      <p className={`text-sm leading-relaxed transition-colors duration-300 ${(showTennis || showPakistan || showQatar) ? 'text-gray-400' : 'text-foreground text-muted'}`}>
        I love <span className="text-sm text-foreground">football </span> (in other words 'soccer'), <button onClick={() => { setShowTennis(!showTennis); setShowPakistan(false); setShowQatar(false); }} className={`text-sm hover:underline cursor-pointer transition-colors duration-300 ${showTennis ? 'text-green-500' : 'text-foreground'}`}>tennis</button>, and <span className="text-sm text-foreground">padel</span>.
      </p>
