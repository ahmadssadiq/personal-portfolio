...
const MainContent = ({ showTennis, setShowTennis, showPakistan, setShowPakistan, showQatar, setShowQatar }: MainContentProps) => {
  return (
    <div className="flex flex-col space-y-4 break-words overflow-wrap-anywhere">
      <p className={`text-sm leading-relaxed transition-colors duration-300 ${(showTennis || showPakistan || showQatar) ? 'text-muted opacity-60' : 'text-foreground'}`}>
      ...
      {/* <p className={`text-sm leading-relaxed transition-colors duration-300 ${showTennis ? 'text-muted opacity-60' : 'text-foreground'}`}>
